...
`app/api/reservations/route.ts` owns all reservation state. A unique database key prevents duplicate reservations, including simultaneous requests. The four gaiwans are alternatives; the three tea sets are independently reservable. Tea scales and all other gifts are independently reservable.
...
Prices and availability checked 24 September 2026; retailer pages remain authoritative. The replacement HomeCoffee scales were listed in stock at 890 UAH, with no deferred-dispatch notice. The Game WoW mat links to the 400×900 mm option.
